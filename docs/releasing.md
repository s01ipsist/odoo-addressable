...
## The model: develop on main, generate the rest

- **`main` is the only branch you edit.** It holds the whole module with a
  bare `version` (`1.0.0` — the single source of truth for x.y.z).
- **`17.0` / `18.0` / `19.0` are generated**, never edited by hand. Each is just
  `main` + the manifest `version` rewritten to `<series>.<x.y.z>`.
- They are **force-pushed** from main, so treat them as build artifacts.
...
`.github/workflows/release-branches.yml` runs on every push to `main` and
regenerates + force-pushes the series branches via `tools/release.sh`. So:

- Merge to `main` → the series branches update themselves. Nothing to do.
...
1. Make your changes on `main`; ensure `test/run.sh` passes on each series.
2. Bump `version` (x.y.z) in `addressable_autocomplete/__manifest__.py` per
   semver (increase on any DB-schema change).
3. Push `main`. The branches regenerate as `17.0.<x.y.z>`, etc.
...
1. Add `"20"` to the matrix in `.github/workflows/e2e.yml` and run
   ...
   main over a branch fork; see below).
...
First try to handle it on `main` with a runtime check (as we already do for
...
- Maintain it by rebasing its patch onto main at release time.

Avoid this if at all possible: keeping every series on one `main` is the whole
...
- The series branches are **not** e2e-tested in CI (identical code to main);
  main + PRs are.
